	...
	try:
		if country_code=='PY':
			query="""SELECT serv_x,serv_y,serv_nombre,serv_premium,serv_id from servidores 
				where  serv_premium = true and serv_hacked_faccion = 998 and serv_hacked_user is null and serv_active = true"""
		else:
			query="""SELECT serv_x,serv_y,serv_nombre,serv_premium,serv_id from servidores 
				where  serv_premium = true and serv_hacked_faccion = 998 and serv_country='"""+country_code+"""' and serv_hacked_user is null and serv_active = true"""
		conn = mypool.connect()
		cur = conn.cursor()
		cur.execute(query)
		rows=ResultIter(cur)
...
def bestFhacktions():
	try:
		...
		group by s.serv_hacked_faccion,facc_nivel order by servers_count desc limit 50""")
	except:
		print "connection error"
		return ""
	finally:
...
def bestHackers():
	try:
		...
		order  by sum_exp desc limit 50""")
	except:
		print "connection error"
		return ""
	finally:
...
def listProducts():
	...
	if vtoken=="ok":
		try:
			conn = mypool.connect()
			cursor = conn.cursor()
			cursor.execute("select stpr_web_id,stpr_description,stpr_price,stpr_name,stpr_image_path from store_premium")
		except:
			print "connection error"
			return ""
		finally:
