    ...
    MapPaint.ActionControl = L.Control.extend({
        ...
        onAdd: function (map) {
            ...
            var eraser = L.DomUtil.create('div', 'mappaint-eraser');
             var eraser_clean = L.DomUtil.create('div', 'mappaint-clean');

            eraser_clean.onclick = function () {
                   /* if(_screenshot!=null){
                  map.removeLayer(_screenshot);
              }*/
           ...
           // container.appendChild(eraser_clean);
...
L.MapPaint = L.Handler.extend({
    ...
    saveMethod: function (image, bounds) {
      _screenshot =  L.imageOverlay(image, bounds).addTo(this._map);
